...
export default function CheckoutPage({ orderDetails, onCancel }) {
  ...
  const handleHotelConfirm = async () => {
    setIsProcessing(true);
    try {
      await fetch(`/api/orders/${orderDetails.orderId}/confirm-payment`, {
        method: "POST",
      ...
      <div className="min-h-screen bg-washwell-cream font-body flex items-center justify-center p-6">
        <div className="max-w-2xl w-full">
          ...
          <div className="bg-white rounded-3xl shadow-2xl border-2 border-washwell-gray-light p-10 text-center">
            <div className="w-24 h-24 bg-washwell-green rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg">
              <svg className="w-12 h-12 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
              </svg>
            </div>
            <h3 className="text-2xl font-display font-bold text-washwell-green mb-2">Order Confirmed</h3>
            <p className="text-washwell-gray-dark">Your pickup has been scheduled. We'll be in touch shortly.</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-washwell-cream font-body flex items-center justify-center p-6">
      <div className="max-w-2xl w-full">
        ...
        <div className="bg-white rounded-3xl shadow-2xl border-2 border-washwell-gray-light p-10">
          ...
          {/* Stripe — just open the link, webhook handles confirmation */}
          {paymentMethod === "stripe" && (
            <div className="space-y-3">
              <a
                href={`${orderDetails.stripeUrl}?client_reference_id=${orderDetails.orderId}`}
                target="_blank"
              ...
              <p className="text-xs text-washwell-gray text-center">
                Your order will be confirmed automatically once payment is complete.
              </p>
            </div>
          )}
          ...
          {paymentMethod === "cash" && (
            <div className="space-y-3">
              <div className="bg-yellow-50 border-2 border-yellow-200 rounded-xl p-4 text-sm text-yellow-800">
                Please have <span className="font-bold">${orderDetails.estimatedTotal}</span> ready to pay the driver at pickup.
              </div>
              <button
              ...
              >
                {isProcessing ? "Confirming..." : "Confirm — Cash on Pickup"}
              </button>
            </div>
          )}
          ...
          {paymentMethod === "hotel_account" && (
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full py-5 bg-washwell-green hover:bg-washwell-green-dark text-white font-bold rounded-xl shadow-lg transition-all disabled:opacity-50 text-lg"
              >
                {isProcessing ? "Confirming..." : "Confirm Hotel Charge"}
              </button>
            </div>
          )}
